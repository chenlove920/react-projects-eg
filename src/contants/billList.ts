...
export const billTypeToName = (type: string) => {
  return Object.values(billListData).reduce((prev, billList) => {
    //
    billList.filter(items => {
      const searchData = items.list.filter(item => item.type === type)
      if (searchData.length > 0) prev = searchData[0].name
    })
    return prev
  }, "")
}
